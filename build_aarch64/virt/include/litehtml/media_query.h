...
namespace litehtml
{
	...
	private:
		...
		/* Set when the query mentions a feature this engine does not know at
		 * all. Per spec such a query never matches; dropping the expression
		 * instead would make the query vacuously true and silently apply
		 * e.g. print-only or exotic-device styles. */
		bool							m_unknown;
	public:
		media_query();
