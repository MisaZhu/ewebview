...
protected:
	...
	public:
		...
		bool						m_step_done;
		/* Set once stylesheets (master + document sheets) have been matched
		 * against this element. Nodes from document::create_element start
		 * unstyled; document::style_detached_subtree uses the flag to style
		 * exactly the freshly inserted subtree once, leaving already matched
		 * static nodes (e.g. a moved subtree) untouched. */
		bool						m_sheets_applied;
		bool						sheets_applied() const { return m_sheets_applied; }
		void						set_sheets_applied(bool v) { m_sheets_applied = v; }

		/* Liveness tag for the JavaScript DOM bridge. The embedder hands
		 * litehtml::element* pointers to the VM as opaque handles; a wrapper can
		 * outlive the element it wrapped (freed on a document swap or an
		 * innerHTML rewrite) and the VM may recycle that memory for its own
		 * objects. The bridge calls is_live_handle() before dereferencing a
		 * returned handle, so a dangling pointer degrades to null instead of
		 * being spliced into the tree and aborting in the style walk. Set in the
		 * constructor, cleared in the destructor. An enum keeps the constant a
		 * pure prvalue (no out-of-line definition / ODR use). */
		enum { ELEMENT_LIVE_MAGIC = 0x4C48544D }; /* "LHTM" */
		unsigned int				m_el_magic;
		bool						is_live_handle() const { return m_el_magic == (unsigned int)ELEMENT_LIVE_MAGIC; }
		...
		/* Insert `el` immediately before `ref` among this element's children.
		 * A null `ref` (or one that is not a child of this element) appends,
		 * matching DOM insertBefore semantics. Added for the JavaScript DOM
		 * bridge, which needs ordered insertion but must not reach into the
		...
		/* Drop an attribute entirely so get_attr falls back to `def` again.
		 * set_attr(name, "") cannot express this: it leaves the attribute
		 * present with an empty value, which the DOM bridge must report as ""
		 * rather than as absent. Added for removeAttribute(). */
		virtual void				remove_attr(const tchar_t* name);
