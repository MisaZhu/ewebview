...
#define  font_weight_strings	_t("normal;bold;bolder;lighter;100;200;300;400;500;600;700;800;900")
...
#define element_position_strings	_t("static;relative;absolute;fixed;sticky")
	...
	enum element_position
	{
		...
		/* sticky lays out in flow like static/relative: this engine has no
		 * scroll-driven pinning, and falling back to fixed would stack every
		 * sticky element on top of the viewport. */
...
#define media_feature_strings		_t("none;width;min-width;max-width;height;min-height;max-height;device-width;min-device-width;max-device-width;device-height;min-device-height;max-device-height;orientation;aspect-ratio;min-aspect-ratio;max-aspect-ratio;device-aspect-ratio;min-device-aspect-ratio;max-device-aspect-ratio;color;min-color;max-color;color-index;min-color-index;max-color-index;monochrome;min-monochrome;max-monochrome;resolution;min-resolution;max-resolution;prefers-color-scheme;prefers-contrast;prefers-reduced-motion;forced-colors;pointer;any-pointer;hover;any-hover;display-mode")
	...
	enum media_feature
	{
		...
		/* Environment queries: evaluated against this browser's fixed
		 * capabilities (light scheme, fine pointer, hover, plain browser
		 * window). Unknown features outside this list make the whole
		 * query false, they are NOT vacuously true. */
		media_feature_prefers_color_scheme,
		media_feature_prefers_contrast,
		media_feature_prefers_reduced_motion,
		media_feature_forced_colors,
		media_feature_pointer,
		media_feature_any_pointer,
		media_feature_hover,
		media_feature_any_hover,
		media_feature_display_mode,
	};
