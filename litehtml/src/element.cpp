...
{
	/* Clear the liveness tag first so a JS wrapper still holding this handle is
	 * rejected by element::is_live_handle() the instant the node dies. */
	m_el_magic		= 0;
}
...
void litehtml::element::remove_attr( const tchar_t* name )					LITEHTML_EMPTY_FUNC
void litehtml::element::apply_stylesheet( const litehtml::css& stylesheet )
{
	/* Non-html_tag nodes (el_text, el_comment) carry no selector-matchable
	 * state, but they must still be marked styled so a later
	 * style_detached_subtree() walk does not revisit them. */
	(void)stylesheet;
...
litehtml::element::ptr litehtml::element::clone_node(bool deep)						LITEHTML_RETURN_FUNC(0)
