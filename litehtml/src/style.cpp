...
void litehtml::style::add_property( const tchar_t* name, const tchar_t* val, const tchar_t* baseurl, bool important )
{
	...
	/* Map CSS logical box properties onto their physical equivalents for the
	 * only supported writing mode (horizontal-tb / ltr). Modern stylesheets
	 * space flex rows with e.g. "li + li { margin-inline-start: .5rem }"; the
	 * logical name is otherwise unknown here and silently dropped, which
	 * leaves the w3.org global nav items butted together. */
	if(!t_strcmp(name, _t("margin-inline-start")))
	{
		name = _t("margin-left");
	} else
	if(!t_strcmp(name, _t("margin-inline-end")))
	{
		name = _t("margin-right");
	} else
	if(!t_strcmp(name, _t("margin-block-start")))
	{
		name = _t("margin-top");
	} else
	if(!t_strcmp(name, _t("margin-block-end")))
	{
		name = _t("margin-bottom");
	} else
	if(!t_strcmp(name, _t("padding-inline-start")))
	{
		name = _t("padding-left");
	} else
	if(!t_strcmp(name, _t("padding-inline-end")))
	{
		name = _t("padding-right");
	} else
	if(!t_strcmp(name, _t("padding-block-start")))
	{
		name = _t("padding-top");
	} else
	if(!t_strcmp(name, _t("padding-block-end")))
	{
	   ...
	   !t_strcmp(name, _t("margin-block")) || !t_strcmp(name, _t("padding-block")))
	{
		const tchar_t* a;
		const tchar_t* b;
		if(!t_strcmp(name, _t("margin-inline")))
		{
			a = _t("margin-left"); b = _t("margin-right");
		} else
		if(!t_strcmp(name, _t("padding-inline")))
		{
			a = _t("padding-left"); b = _t("padding-right");
		} else
